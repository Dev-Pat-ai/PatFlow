package com.patflow.app.domain.usecase.payment;

import com.patflow.app.domain.repository.PaymentRepository;
import dagger.internal.DaggerGenerated;
import dagger.internal.Factory;
import dagger.internal.QualifierMetadata;
import dagger.internal.ScopeMetadata;
import javax.annotation.processing.Generated;
import javax.inject.Provider;

@ScopeMetadata
@QualifierMetadata
@DaggerGenerated
@Generated(
    value = "dagger.internal.codegen.ComponentProcessor",
    comments = "https://dagger.dev"
)
@SuppressWarnings({
    "unchecked",
    "rawtypes",
    "KotlinInternal",
    "KotlinInternalInJava",
    "cast",
    "deprecation"
})
public final class GetPaymentDetailUseCase_Factory implements Factory<GetPaymentDetailUseCase> {
  private final Provider<PaymentRepository> repositoryProvider;

  public GetPaymentDetailUseCase_Factory(Provider<PaymentRepository> repositoryProvider) {
    this.repositoryProvider = repositoryProvider;
  }

  @Override
  public GetPaymentDetailUseCase get() {
    return newInstance(repositoryProvider.get());
  }

  public static GetPaymentDetailUseCase_Factory create(
      Provider<PaymentRepository> repositoryProvider) {
    return new GetPaymentDetailUseCase_Factory(repositoryProvider);
  }

  public static GetPaymentDetailUseCase newInstance(PaymentRepository repository) {
    return new GetPaymentDetailUseCase(repository);
  }
}
